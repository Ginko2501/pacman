...
(* [state] is the state of a voxel [Player] is the player state [Bot] is the bot
   state [Wall] is the wall state [Empty] is the empty state [Dot] is the dot
   state *)
type state =
  | Player
  | Bot
...
(* [plot v] plots [v] on the graphics window. If the state of v is [Player],
   ...
   drawn. If the state of v is [Wall], then a blue square is drawn. If the state
   of v is [Empty], then a black square is drawn. If the state of v is [Bot],
   then a red circle is drawn. *)
let draw (v : t) =
  match v.s with
  | Player -> ()
  | Dot ->
